...
The workflow inputs are `artifact_name`, `project`, `platform`, `channel`, `runtime_version`, and `keyid`; `runtime_version` is required and `keyid` is optional. The required `signing_private_key` workflow secret is passed by the caller and maps to the publisher's signing input. The workflow maps `EXPO_OTA_PUBLIC_BASE_URL`, `EXPO_OTA_R2_BUCKET`, and `CLOUDFLARE_ACCOUNT_ID` into the publisher's required service inputs, while Vault supplies only the R2 access key ID and secret access key. The root Action metadata describes the bundled Node 24 JavaScript Action; the reusable workflow currently executes the checked-out bundle directly. The R2 access key ID, secret access key, and signing key are publishing credentials; callers must never commit them. `keyid` is fixed when the manifest is published and is not combined with `runtime_version` for secret storage.
